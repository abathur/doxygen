...
struct Refid insertRefid(const char *refid)
{
  ...
  bindTextParameter(refid_select,":refid",refid,FALSE);// TODO: check insertRefid calls for .data
  ret.rowid=step(refid_select,TRUE,TRUE);
  if (ret.rowid==0)
  {
    bindTextParameter(refid_insert,":refid",refid,FALSE);// TODO: check insertRefid calls for .data
...
static bool insertMemberReference(struct Refid src_refid, struct Refid dst_refid, const char *context)
{
  ...
  {
    bindTextParameter(xrefs_insert,":context",context,FALSE); //TODO: check calls for .data
  }
